...
/**
 * @param {any} sender 飞书事件里的 sender 对象
 * @returns {string[]} open_id / user_id / union_id 里非空的部分
 */
...
/**
 * @param {string[]} ids
 * @param {Record<string, string>|null} keyMap
 * @returns {string}
 */
...
/**
 * @param {string[]} ids
 * @param {Record<string, string>|null} legacyToFeishu
 * @returns {string}
 */
...
function mentionIsBot(message, botOpenId, botName) {
  ...
  return mentions.some(/** @param {any} m */ function (m) {
    const id = (m && m.id) || {};
